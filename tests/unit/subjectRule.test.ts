...
describe('subjectText và softmax', () => {
  it('chữ hiển thị kèm phần trăm; unknown không có phần trăm', () => {
    expect(subjectText({ subjectType: 'person', confidence: 0.934 })).toBe('Người 93 %')
    expect(subjectText({ subjectType: 'mannequin', confidence: 0.8 })).toBe('Hình nộm 80 %')
    expect(subjectText({ subjectType: 'unknown', confidence: 0.5 })).toBe(
      'Khuôn mặt chưa phân loại',
    )
    expect(subjectText({ subjectType: 'person' })).toBe('Người')
  })
