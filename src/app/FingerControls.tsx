...
export function FingerControls({ store }: { store: StageStore }) {
  ...
  const lang = useLang()
  const f = useStrings().settings.fingers
  ...
  return (
    <section className="sec" data-testid="fingers-bar">
      <h3>
        {f.title} <span className="hint">{f.both}</span>
      </h3>
      <div className="chips">
        {TIP_CHOICES.map((t) => (
      ...
      <span className="hint">{f.hint(DEFAULTS.reveal.minPoints, DEFAULTS.hands.minHands)}</span>
    </section>
  )
}
