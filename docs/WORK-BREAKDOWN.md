...
Depends on: GRID-01 (layout), MASK-01 (compositor order and the video clip), PERF-02 (static-frame skip), UX-04 (Interface section, `uiState`), I18N-01 (strings in both dictionaries). Planned and implemented on 2026-09-21 (D-056), revised the same day after the user's correction (D-057): the black area of the mock-up is the open region, not the logo, and the small logo in its corner is the logo itself. Revised again the same day (D-058): the frames of the logo must lie on the cell lines, so the logo is placed on the board grid by a module of k cells.

Motivation: the closed cover is a plain white grid. The campaign logo ("VERIFY: Human – AI Ethic Campaign") is drawn on it at a corner, at a size that does not depend on the cell count, and the reveal region cuts it cell by cell: where a cell is open the camera shows instead of that piece of the logo, so the cut edges follow the cell boundaries ("inlaid", as in the mock-up where the open region hides half of "Human").

Measurements (2026-09-21; the raster and the mock-up with `tools/measure-logo.mjs`, the vector by reading its markup):

| Item | Value | Use |
|---|---|---|
| Vector source `src/assets/logo-verify-human.svg` (the designer's `Artboard 10.svg`) | viewBox 700 × 400; three frames as `<rect>` with a 3.78 px navy stroke centered on the edge: "VERIFY:" (231.73, 92.8, 391.29 × 77.93), "Human" (75.65, 170.73, 391.29 × 77.93), "AI ETHIC CAMPAIGN" (309.77, 248.66, 313.25 × 49.81); the union of their outer edges is (73.76, 90.91, 551.15 × 209.45), aspect 2.631; two ink colors, navy `#1f3566` (text, frames) and green `#99b43e` ("VERIFY:"); "VERIFY:" and "Human" are outlined paths, "AI ETHIC CAMPAIGN" is live `<text>` in the font Heavitas with per-glyph `tspan` offsets | The crop and the frame rectangles are hard-coded in `core/brandLogo.ts` from the rect attributes (a unit test parses the bundled SVG and compares); the same SVG is the image drawn on the cover; the live text needs a fallback because Heavitas is installed neither on the development machine nor on a kiosk |
| Module grid of the frames (D-058) | With the unit u = frame width / 15 = 26.09 viewBox units, the three frames sit on a 21 × 8 grid: "VERIFY:" 15 × 3 cells from column 6, "Human" 15 × 3 cells from column 0, "AI ETHIC CAMPAIGN" 12 × 2 cells from column 9, row 6; the largest deviation of any edge is 0.09 cell (the campaign frame height 1.91 cells → 2); the 21 : 8 aspect is 2.625 against the true 2.631 | Lets the logo be drawn with every frame edge on a board cell line when the module is a whole number of cells; a unit test compares `LOGO_FRAME_CELLS` with the SVG rectangles |
...
| Mock-up | 660 × 375 px, cell pitch ≈ 19 to 20 px (about 33 × 19 cells); a black region with bbox 489 × 234 px (74 % × 62 % of the image) whose edges follow the cells; the original logo at the top-left corner, about 20 % of the image wide, with "Human" cut off where the black region starts | The black region stands for the open reveal region (camera), the logo is the original artwork on the cover, and the cut between them follows the cells: exactly what the video clip of MASK-01 produces when the logo is drawn under it |
| Stage 1280 × 720 | cell 40 / 20 / 10 px at the 32 / 64 / 128 presets; the logo at 30 % of the stage width is 384 × 146 px in every preset (unit test), 16 % of the stage height | The size invariance to check. E2E at the headless 960 × 540 stage: 288 × 109 px at all three presets (cells 30 / 15 / 7 px) |

Design (D-056, revised by D-057):

- Placed on the board grid (D-058). `logoGeometry` chooses the module k = max(1, round(`widthRatio` (0.3) × stage width / (21 c))) and gives the logo 21 k × 8 k cells at the anchor (`top-left`) with a margin of round(`marginRatio` × stage width / c) cells, clamped into the board; the three frames are `LOGO_FRAME_CELLS` × k c, so every frame edge is a cell line. The width is therefore the multiple of 21 cells nearest to 30 % of the stage: 420 px at 64, 128 and 256 columns on a 1280 px stage (k = 1, 2, 4), which keeps the size independent of the cell count wherever the module divides evenly. When 21 k cells would exceed `snapMaxWidthRatio` (0.5) × stage width (32 columns: 840 px), the height cap or the board (fewer than 21 × 8 cells), the logo falls back to the fixed rectangle of D-057 (`logoRect`: 30 % of the stage width, margin from the stage edge, clamped into the board) with the frames in the SVG proportions and no cell alignment.
- The artwork as is. `logoSvg()` prepares the bundled SVG for `Image`: frames, paths and colors are untouched; the live `<text>` with its `tspan`s becomes one flat `<text>` with `textLength="268"`, `lengthAdjust="spacingAndGlyphs"` and an inline `font-family` (Heavitas first, then 'Arial Black', 'Segoe UI Black', sans-serif); the root gets `width`/`height` equal to the viewBox (Chrome's intrinsic size of an SVG without them is 300 × 150 scaled by the aspect, which shifts the source rectangle of `drawImage`). `mask/logoLayer.ts` rasterizes the logo once per layout into an `OffscreenCanvas` (the rectangle plus a pad for the half stroke) and the compositor draws it with one 9-argument `drawImage`. Snapped: each frame is its own `drawImage` from the frame's SVG rectangle (plus half the stroke) to the frame's cell rectangle (plus the scaled half stroke), so the SVG's own stroke lands centered on the cell line and the content inside scales with its frame (the campaign frame is 4.8 % taller than in the SVG, the others 0.4 %); not snapped: one `drawImage` of the crop.
- Draw order in the compositor: white → grid lines → logo → video in the clip of the open cells → overlays. The logo is drawn after the lines so its frame strokes cover the cell lines they sit on, while its transparent parts still show the lines (it is part of the tiles); the video clip covers every open cell, so the region cuts the logo cell by cell with no extra geometry, and closing the region brings the logo back on the next paint. Nothing is hidden by time or state (the plan's `hide` with a reappear delay and the block silhouette of the first implementation were removed).
- Not a camera pixel (I4): the layer's only `drawImage` sources are the `Image` of the bundled SVG and its own canvas; `tools/check-invariants.mjs` lists the module with the same 9-argument rule (lint:boundaries keeps `mask/` away from the camera). Nothing leaves the browser (I9): the SVG is a `?raw` string in the bundle turned into a blob URL.
- Cost: cover closed → the logo is part of the static paint (the PERF-02 skip is unchanged: zero paints per second, measured +0 over 20 frames); cover open → one `drawImage` of the cached canvas per painted frame; the canvas is rebuilt only when the layout object or the image version changes; the rectangle is cached per layout in a `WeakMap`.
- Settings: `ui.logo` (boolean, default `DEFAULTS.brand.logo.enabled` = true) in `uiState` (the tab's `wct.ui`, no epoch), switch "Logo trên màn che" (Logo on the cover) in the Interface section, mirrored into `LogoLayer.enabled`; `?logo=0|1` on the URL overrides the stored value for that load (`useUiState` overrides). Scenario `logo(0|1)`; `window.__wct.logo` (`enabled`, `visible`, `ready`, `rect`, the three frame rectangles in stage px) and the `logo-stat` line of the debug drawer.
...
1. `core/brandLogo.ts` (pure): `LOGO_FRAMES_SVG`, `LOGO_STROKE`, `LOGO_CROP`, `LOGO_ASPECT`, `LOGO_FRAMES` (normalized), `LOGO_GRID` and `LOGO_FRAME_CELLS` (the 21 × 8 module grid), `logoRect(stage, board, placement)`, `logoFramesIn(rect)`, `logoGeometry(grid, placement)` (snapped or fixed), `logoSvg(source, { fonts, textLength })`; `DEFAULTS.brand.logo` (`enabled`, `widthRatio`, `maxHeightRatio`, `marginRatio`, `anchor`, `snapMaxWidthRatio`, `fonts`, `textLength`).
2. `mask/logoLayer.ts`: `LogoLayer` (SVG string in, `Image` from a blob URL, `ready`/`version`, `enabled`, `geometry(layout)` cached per layout, `draw(ctx, layout)` from a cached `OffscreenCanvas`; per-frame `drawImage` when snapped).
3. Compositor: `LogoPainter` type, `paintBackground(ctx, layout, showLines, logo)` draws white → lines → logo, `RenderOptions.logo`.
4. Loop: `deps.logo()`, `paintLogo`/`paintLogoVersion` in the static-paint key, `closeNow` paints with the logo, `LoopSnapshot.logoVisible` (switch on).
5. UI and debug: `uiState.logo`, `useUiState(defaults, overrides)`, the Interface switch, `?logo=`, strings in both dictionaries; `debug/logoProbe.ts` (`window.__wct.logo` with `rect`, `frames`, `snapped`, `module`; `describeLogo`), the `logo-stat` line, scenario `logo`; `StagePage` builds the layer from the `?raw` import and passes it to the loop, the first paint, the probe and the scenarios.
6. E2E helpers: `openApp` appends `logo=0` unless the query sets it and `APP_URL` accepts a query; `readLogo`; `logo.spec.ts` turns the logo on with `logo=1`. Tests of 7.33; documentation: README, 9.1, D-056/D-057, `tools/README.md`, `tools/test-report.mjs`.
7. Out of scope: the same logo in the landing preview.

Done criteria: with the cover closed, at 64 and 128 columns the logo is 21 k × 8 k cells with every frame edge a multiple of the cell size from the board origin, the midpoints of the outer frame edges are exactly navy and 3 px outside them white or grid gray, and the width is within 15 % across the presets; at 32 columns the logo keeps the fixed 30 % width without alignment; green ink inside the "VERIFY:" frame and navy ink inside the "Human" frame, no logo color beside or below the rectangle, and the corners and mid-edges of the canvas white or grid gray; a mouse window over the "Human" frame shows the camera in its cells (no logo ink inside, apart from the window outline) while the column of cells beside it still has logo ink and the "VERIFY:" frame is intact; closing the window restores the ink count; the switch, `?logo=0` and the stored value behave as specified; every existing e2e passes unchanged; `paints` stays at zero with the cover closed; no request leaves the origin.
...
| Rectangle | Unit `brandLogo`: 30 % of the stage width, aspect, 2.5 % margin, top-left anchor → (32, 32, 384 × 146) on 1280 × 720, identical for the 32/64/128/256 16:9 grids; on a letterboxed 1000 × 600 stage the size and margin are equal at every preset and only the clamp to the board moves it; the five anchors; a board inset deeper than the margin clamps to the board; `maxHeightRatio` caps a wide logo; `logoFramesIn` of a rectangle unions back to it; empty board or stage → null | unit | BRAND-01 |
| Module grid | Unit `brandLogo`: `LOGO_GRID` is 21 × 8 and the three `LOGO_FRAME_CELLS` reproduce the SVG frame rectangles within 0.1 cell on every edge (the worst edge above 0.05 cell, so the tolerance is real) and union to the full grid | unit | BRAND-01 |
| Geometry | Unit `brandLogo` (`logoGeometry`): at 64, 128 and 256 columns on 1280 × 720 the logo is snapped, 420 × 160 px, module 1, 2 and 4 cells, margin 2 or 3 cells, every frame edge a multiple of the cell size, the first frame at columns 6–21 rows 0–3 and the union of the frames equal to the rectangle; at 32 columns (21 cells = 840 px) it falls back to the fixed 384 × 146 rectangle with proportional frames and snaps again when `snapMaxWidthRatio` is raised; the right, bottom and center anchors count cells from the board edges; a 20 × 20 grid or `c = 0` falls back; an empty board → null | unit | BRAND-01 |
| Logo SVG | Unit `brandLogo` (`logoSvg` on the bundled file): the three frame rects, the two green colon squares, every path, both fills and the navy stroke are kept; no `tspan`; one flat `<text>` "AI ETHIC CAMPAIGN" with the original class and transform, `textLength="268"`, `lengthAdjust`, inline Heavitas-first font stack; the root gains `width="700" height="400"`; everything else is byte-identical to the source; an SVG that already has a width is left alone; the source string is unchanged | unit | BRAND-01 |
| Compositor order | Unit `compositor`: the logo painter is called once, after the white fill and all cols + rows + 2 grid-line `fillRect`s and before the video `drawImage`; `logo: null` adds no call; with lines off and no mask the calls are exactly white then logo | unit | BRAND-01 |
| Config, uiState | Unit: `brand.logo` ranges (ratios in (0, 1], `snapMaxWidthRatio` ≥ `widthRatio`, anchor in its set, Heavitas-first font stack, `textLength` > 0); `readUiState` with `logo` (default, boolean kept, garbage → default, old records without the key) | unit | BRAND-01 |
| Snapped size and ink | E2E `logo.spec` (`?logo=1`, cover closed): at 64 and 128 columns `__wct.logo` is snapped with module k c, the rectangle is 21 k × 8 k cells at the margin in cells from the board origin, every frame edge is a multiple of the cell size, the midpoints of the outer edges ("VERIFY:" top and right, "Human" left, campaign bottom) are exactly navy and 3 px outside them white or grid gray; at 32 columns the logo is not snapped and 30 % of the stage width; widths across the three presets within 15 %; more than 50 green and 50 navy px inside the "VERIFY:" frame, more than 50 navy and no green px inside the "Human" frame, no logo color in a 40 px strip right of and below the rectangle; the nine samples of `expectCanvasWhite` are white or grid gray; `paints` +0 over 20 loop frames; sizes and modules recorded | e2e | BRAND-01 |
| Cut by the region | E2E `logo.spec`: a mouse window covering the left half of the "Human" frame → the center of an open cell is the synthetic magenta, the window interior (4 px inside the outline) has magenta on more than half of its pixels and no green, navy or white, the column of cells beside the window still has navy ink and no magenta, the "VERIFY:" frame keeps its green; `logo-stat` says "hiện"; gate audit clean; `coverAll` → no magenta in the frame and the navy count returns within 2 px of the count before; no request leaves the origin (blob URLs are in-page); counts recorded | e2e | BRAND-01 |
| Switch and URL | E2E `logo.spec`: the "Logo trên màn che" switch off → `enabled` false, `logoVisible` false, no ink in the "Human" frame and its center white, `logo-stat` "logo: tắt", `wct.ui.logo` false; a reload in the same tab without the parameter keeps it off; `?logo=1` turns it on and `?logo=0` off (canvas white); the English label "Logo on the cover" with `?lang=en` | e2e | BRAND-01 |
| Existing suite | Every existing e2e passes unchanged: `openApp` adds `logo=0` and `APP_URL` accepts the query; the `synthetic.spec` scenario list gains `logo`; the landing-page start flows (logo on, top-left) still pass `expectCanvasWhite` because its nine samples never touch the corner logo | e2e | BRAND-01 |
