...
| Cover logo | The campaign logo is inlaid into the closed cover as cell-aligned navy blocks with the wordmark on them (white "Human" and "AI ETHIC CAMPAIGN", green "VERIFY:"): the silhouette is the union of the three frames of the logo (`core/brandLogo.ts`, from the bundled SVG), the rectangle is 30 % of the stage width at the top-left corner with a 2.5 % margin and does not depend on the cell count, a cell is a logo cell when at least half of it lies under the silhouette; hidden while a region is open and back 800 ms after the close | BRAND-01 (D-056); `brand.logo` in `config.ts`; `ui.logo` switch "Logo trên màn che", `?logo=0|1`; the campaign line renders in a fallback font until the SVG is re-exported with outlined text |
...
flowchart TD
  ...
  FR -- yes --> RZ{"raised-only on and the finger folded per fingerPose? (ROI-04)"}
  RZ -- yes --> X4["point: folded"]
  RZ -- no --> SC{"score ≥ minTrackScore?"}
  ...
  ALL -- no --> R2["few-points, or the dominant reason among invalid points (out-of-board before stale-point before folded)"]
...
4. CI and the `deploy` job (done, not yet run for real because there was no repo). The `check` job builds with `VITE_BASE` from the repository variable `vars.PAGES_BASE`, default `/<repo>/` from `github.event.repository.name`; after `build`: check `test ! -e dist/spike-assets`, `dist/sw.js` has no placeholder left, print the size; run `npm run test:deploy` on that same `dist/`; `upload-pages-artifact@v5` on push to `main` (PRs only keep the `dist` artifact for 7 days). The `deploy` job: `needs: check`, push to `main` only, `permissions: { contents: read, pages: write, id-token: write }`, `environment: github-pages` with `url` from `deploy-pages`, `concurrency` group `pages` without cancellation, `configure-pages@v6` then `deploy-pages@v5`, the address printed in the summary. Workflow concurrency only cancels old runs for pull requests, not on `main`, so a deploy is never cut short. No `.nojekyll` or `CNAME` needed in `public/`. Rollback: rerun the `deploy` job of the old commit in the Actions tab, or revert and push.
...
#### ROI-04 Raised fingers only: the region follows the fingertips that are actually shown

Depends on: ROI-03 (fingertip evaluation, hull solver), PERF-02 (mask reuse), I18N-01 (new strings in both dictionaries); calibration on a webcam is the R-02 session still owed from INT-01. Planned and implemented on 2026-09-21 (D-055); the webcam calibration (step 0 second half and the last row of 7.32) is still owed.

Motivation: since ROI-03 every selected fingertip of every tracked hand is a hull candidate whether or not the finger is raised. MediaPipe predicts all 21 landmarks even for a folded or hidden finger, and the predicted tip of a folded finger lands in the palm, so folding a finger does not remove a vertex: it pulls the hull inward or leaves a phantom vertex, and the only way to shape the region with fewer fingers is the static "Đầu ngón dùng" chip bar, which applies to both hands. The requested behavior: a fingertip takes part only while the finger is shown and not folded or hidden; for example three raised fingers on one hand and two on the other give a five-point polygon, and folding one more finger shrinks it.

Measurements (2026-09-21, `tools/probe-fingers.mjs` on the spike images, hand_landmarker float16/1, IMAGE mode, CPU delegate; 4 hands, 20 fingers: 14 raised, 6 folded):

| Metric | Raised fingers | Folded fingers | Use |
|---|---|---|---|
| `visibility` of the tip landmark | 0 for all 20 | 0 | Not populated by the hand landmarker; unusable |
| r3d = \|wrist→tip\| / \|wrist→PIP\| on the world landmarks (meters) | 1.19 to 1.34 (thumb 1.51 to 2.21) | 0.61 to 0.83 | Primary rule, orientation and scale invariant |
| r2d, the same ratio in image px | 1.22 to 1.34 | 0.58 to 0.70 | Fallback when world landmarks are missing (fake hands, old results) |
| angle at the PIP joint, world landmarks | 144° to 175° | 74° to 104° | Second condition for the four fingers; unusable for the thumb (147° on the thumb folded across the palm in `pointing_up.jpg`) |
| tip inside the 2D palm polygon (landmarks 0, 1, 5, 9, 13, 17) | false for all 14 | true for 7 of 8 including the folded thumb | Self-occlusion signal; the one miss is the little finger of `thumbs_up.jpg`, caught by r3d 0.83 and 99° |
| thumb abduction = \|tip→little MCP\| / \|IP→little MCP\| | 0.92 to 1.18 | 0.75 (the tucked thumb) | Thumb rule |

Not in the samples: a thumb folded over an open palm, fingers pointing at the camera (foreshortening, where r2d collapses but r3d holds), a hand seen from the back, and a finger hidden by an object (the model still predicts a plausible tip; only the palm test and the ratio can catch it). These are the cases the webcam session must cover before the thresholds are locked; the fixtures captured with `--json` are the unit-test ground truth.

Design (D-055): "raised" is decided purely from geometry, per track and finger, with hysteresis and a debounce, and a fingertip is a hull candidate only while raised. The convex hull, `minPoints` 3 and `minHands` 2 stay (D-047): 3 + 2 raised fingers give the convex hull of five points (an interior point simply is not a vertex), 1 + 2 still opens, folding down to fewer than 3 points or one hand closes with the usual reasons. A folded finger is a new `FingerReason` `folded`, excluded like a stale point (never a synthetic point). Membership changes do not bump the epoch (as points leaving already do not); the hull, One Euro per point and the cell hysteresis absorb the jump. The chip bar stays as an optional cap, and a new switch "Chỉ ngón đang giơ" (Raised fingers only) in the "Cửa sổ" section, on by default, turns the rule off to get the ROI-03 behavior (every selected tip), which keeps the geometry e2e and the two-finger cases unchanged. Nothing new leaves the browser (I9): world landmarks are numbers computed in the worker that already receives the raw frame.

Rules (`hands/fingerPose.ts`, thresholds in `DEFAULTS.hands.pose`, provisional until the webcam calibration):

- Four fingers: raised when r3d ≥ 1.05 and the PIP angle ≥ 135° and the tip is outside the palm polygon; folded when r3d < 0.9 or the angle < 120° or the tip is inside the palm polygon; in between the previous state is kept (Schmitt trigger). Without world landmarks: r2d with the same bounds and no angle.
- Thumb: raised when abduction ≥ 1.0 and the tip is outside the palm polygon; folded when abduction < 0.85 or the tip is inside the palm polygon; in between the previous state is kept.
- Debounce: a state flips only after `debounceFrames` (3) consecutive HandFrames on the other side (about 100 ms at 30 Hz, below the 150 ms point age); a new track takes its first classification at once so a hand entering the frame opens without delay.
- A missing landmark or a degenerate palm (zero length) keeps the previous state, or raised on the first frame (fail-open keeps ROI-03 behavior).

Steps:

0. Measurement: `tools/probe-fingers.mjs` (done with this plan; `--json` dumps the 2D and world landmarks); capture `tests/unit/fixtures/hands-pose.json` from the three spike images (4 hands, labeled by hand: raised and folded per finger) as the ground truth of step 2; extend the script with a webcam capture mode for the R-02 session (a snapshot per pose: open hand, fist, thumb folded over the palm, fingers toward the camera, back of the hand) and add those hands to the fixture.
1. Protocol: `HandResult.hands[i].worldLandmarks` (21 × [x, y, z] meters) from `hand.worker.ts`; `resultToDetections` → `HandDetection.landmarksWorld`; `HandTrack.landmarksWorld` (optional: absent for fake hands without a world model and for the compositor, which keeps using `landmarksCam`); `hands.spec` checks the field is present with real hands.
2. `hands/fingerPose.ts` (pure, no DOM): `FingerPose = { raised: boolean; r: number; angle: number | null; abduction: number | null; inPalm: boolean; streak: number }`, `classifyFingers(track, prev: Record<FingerTip, FingerPose> | null, opts)` implementing the rules above with `pointInPolygon` from `core/cells.ts`; unit tests on the fixture (every finger of every hand right on the first frame), the hysteresis band (a value inside the band keeps the state in both directions), the debounce (two frames on the other side do not flip, the third does; a new track flips at once), the thumb rule, the 2D fallback, and the degenerate cases.
3. Tracker: `Internal.pose` kept across frames per track (reset with the track, so a re-created id starts fresh), written to `HandTrack.pose`; uncertain frames do not update the pose (the landmarks may belong to the other hand). `window.__wct.hands` exposes the pose per track.
4. `hands/fingertips.ts`: `evaluateFingertips` gets `raisedOnly` (default from config); a tip whose finger is not raised gets `reason: 'folded'`; priority of the dominant reason `out-of-board` > `stale-point` > `folded` > `low-score`; `fingertipsGuidance` adds "N đầu ngón đang gập" (N fingers folded) and, when every seen point is folded, "Xòe các ngón muốn dùng ra" (Spread the fingers you want to use); `describeFingertips` lists `folded` like the other reasons. `HandWindowContext.raisedOnly`; `HandWindowSource` passes it through; the solver is unchanged (it only sees valid points).
5. Settings and config: `DEFAULTS.hands.raisedOnly` (true) and `DEFAULTS.hands.pose` (thresholds, `debounceFrames`); `settings.raisedOnly` in the store (a change bumps the epoch and closes with `config-changed`, like `fingers`); the switch in `GridControls` (section "Cửa sổ", only when the source is hands, `aria-label` "Chỉ ngón đang giơ"); strings in `vi.ts` and `en.ts` (switch, hint, guidance parts); the `config-change` log event carries the key automatically; scenario `raisedOnly(0|1)` for e2e.
6. Drawing and debug: the compositor draws a folded tip as a hollow square (stale and out-of-board stay dim and filled), so the operator sees which fingers the model considers folded; the `fingers-stat` line shows `n folded`; the landing illustration is unchanged.
7. Fake hands: `FakeHandSpec.raised?: FingerTip[]` (default all five); a folded finger gets PIP, DIP and tip offsets bent toward the palm center (tip inside the palm polygon, r2d about 0.6) and fake world landmarks derived from the 2D offsets (z = 0, scaled by spread), so the real classifier runs on fake hands in e2e; `handsAtCell` gets the same option.
8. Tests of section 7.32; documentation: 5.8 flow, 9.1 defaults, README (window paragraph and backlog), `docs/benchmark.md` if the pose costs anything measurable (it is 10 fingers × a few distances per HandFrame, expected below 0.05 ms), D-055 implementation note with the calibration results.

Done criteria: every finger of every fixture hand is classified right on the first frame; fake hands with `raised` [4, 8, 12] on the left and [4, 8] on the right open exactly the convex hull of those five tips (cell set recomputed in Node) and folding the right thumb shrinks it to four points in the same epoch; with the switch off the ten-point cell set of the same fake hands is reproduced; every ROI-03 e2e passes unchanged (fake hands raise all five by default); with the webcam, raising or folding a finger changes the region within the point age plus the debounce and no finger flickers while held still for 10 s; no request leaves the origin.

Tests: section 7.32.

Implementation note (D-055, 2026-09-21): steps 0 to 8 done except the webcam half of step 0. `tests/unit/fixtures/hands-pose.json` holds the four spike hands (2D and world landmarks, `raised` labeled by eye); `hands/fingerPose.ts` (`fingerMetrics`, `fingerVote`, `classifyFingers`, `raisedCount`) with the thresholds in `DEFAULTS.hands.pose`; the worker forwards `worldLandmarks` and `resultToDetections` keeps them only when all 21 are present; `HandTracker` stores `pose` per track (`HandTrack.pose`, `landmarksWorld`), skips uncertain frames and drops it with the track; `evaluateFingertips` takes `raisedOnly` and yields `folded` after out-of-board and stale-point, `fingertipsCloseReason` maps it to `few-points`, `fingertipsGuidance` adds "N đầu ngón đang gập" (N folded fingertips) or the all-folded sentence; `settings.raisedOnly` (epoch++ on change, the loop closes with `config-changed`, `HandWindowSource` resets), the switch in the "Cửa sổ" section, the `raisedOnly(0|1)` scenario; folded tips drawn as hollow squares; fake hands take `raised` per hand, fold the other fingers toward the palm center, carry world landmarks and run the real classifier (the thumb chain offsets 0 to 3 moved so the fake thumb tip at (x, y + spread / 2) passes the abduction rule; every tip offset is unchanged, so the ROI-03 geometry cases are identical). Measured on the fixture: raised fingers ratio 1.19 to 1.34 (the index of hands.jpg 1.1875) and PIP angle 144° to 175° (the ring of hands.jpg 143.97°), folded 0.61 to 0.83 and 74° to 104°; the right thumb of hands.jpg has abduction 0.918, inside the 0.85 to 1.0 band, so it is raised only through the fail-open first classification: the band edges are a calibration item for the webcam session. E2E `solver.spec`: 3 + 2 raised fingers give a five-vertex hull whose cell set equals the Node recomputation, folding the right thumb keeps the epoch and drops the cell count, a fist on one hand closes with `few-points` and "7 đầu ngón đang gập", the switch off restores ten valid points with an epoch bump; `hands.spec` (local assets): `pointing_up.jpg` gives pose raised [8] and `thumbs_up.jpg` raised [4] with `phải 1/5 (4 folded)` and the guidance naming four folded fingertips. Cost: five fingers × a few distances and one polygon test per HandFrame, not measurable in the loop timing.

#### BRAND-01 Campaign logo inlaid into the cover

Depends on: GRID-01 (layout), MASK-01 (compositor order), PERF-02 (static-frame skip), UX-04 (Interface section, `uiState`), I18N-01 (strings in both dictionaries). Planned and implemented on 2026-09-21 (D-056); the user locked the open items the same day: style `wordmark`, hidden while a region is open, placed in a corner, the small original logo of the mock-up not wanted, the vector of the logo provided (`Artboard 10.svg`).

Motivation: the closed cover is a plain white grid. The campaign logo ("VERIFY: Human – AI Ethic Campaign") should be part of it as a mosaic: its edges follow the cell boundaries (inlaid, like the mock-up), its displayed size does not depend on the cell count (a denser grid only makes the edges finer), and while the cover is open the logo is not shown.

Measurements (2026-09-21; the raster and the mock-up with `tools/measure-logo.mjs`, the vector by reading its markup):

| Item | Value | Use |
|---|---|---|
| Vector source `src/assets/logo-verify-human.svg` (the designer's `Artboard 10.svg`) | viewBox 700 × 400; three frames as `<rect>` with a 3.78 px navy stroke centered on the edge: "VERIFY:" (231.73, 92.8, 391.29 × 77.93), "Human" (75.65, 170.73, 391.29 × 77.93), "AI ETHIC CAMPAIGN" (309.77, 248.66, 313.25 × 49.81); the union of their outer edges is (73.76, 90.91, 551.15 × 209.45), aspect 2.631; two ink colors, navy `#1f3566` (text, frames) and green `#99b43e` ("VERIFY:"); "VERIFY:" and "Human" are outlined paths, "AI ETHIC CAMPAIGN" is live `<text>` in the font Heavitas with per-glyph `tspan` offsets | Silhouette and crop hard-coded in `core/brandLogo.ts` from the rect attributes (a unit test parses the bundled SVG and compares); the wordmark layer is the same SVG transformed at runtime; the live text needs a fallback because Heavitas is installed neither on the development machine nor on a kiosk |
| Raster export (first pass, 701 × 401 px) | 21,772 opaque ink px plus 7,614 anti-aliased edge px; the same two colors; frames at (230, 92)–(624, 172), (74, 169)–(468, 250), (308, 247)–(624, 300), i.e. the vector numbers rounded; wordmarks inside their frames: "VERIFY:" (276, 105)–(580, 159), "Human" (86, 173)–(449, 246), "AI ETHIC CAMPAIGN" (330, 264)–(598, 281) | Cross-check of the vector; the campaign line's width 268 px is the `textLength` that the fallback font is forced into |
| Mock-up | 660 × 375 px, cell pitch ≈ 19 to 20 px (about 33 × 19 cells); black blocks with bbox 489 × 234 px (74 % × 62 % of the image, 94,632 px); solid blocks with the grid lines visible across them; a small copy of the original logo at the top-left corner of the blocks | Target look: solid cell-aligned blocks with the grid lines on top; the shape in the mock-up is not the logo's silhouette; the small copy was confirmed as the designer's reference, not part of the result |
| Stage 1280 × 720 | cell 40 / 20 / 10 px at the 32 / 64 / 128 presets; the logo at 30 % of the stage width is 384 × 146 px in every preset (unit test), 16 % of the stage height; the silhouette covers 69.4 % of its rectangle (frame areas minus the two 4 px overlaps), so about 24 / 97 / 389 cells | Expected cell counts; the size invariance to check. E2E at the headless 960 × 540 stage: 288 × 109 px at all three presets, 25 / 97 / 445 cells (cells 30 / 15 / 7 px) |

Design (D-056):

- Fixed size first, cells second. The logo rectangle comes from the stage (canvas) size, not from the board: width = `brand.logo.widthRatio` (0.3) × stage width, height from the aspect 2.631, capped at `maxHeightRatio` (0.6) × stage height, margin `marginRatio` (0.025) × stage width from the stage edge, anchor `top-left`, then clamped into the board (the board is `cols × c` centered on the stage and can be up to one cell smaller, so a board-based size would still change with the preset). It does not read `cols` or `rows`, so every preset shows the same size (the position moves only by the board inset) and the cells only decide how fine the edges are.
- Cells by coverage: a cell is a logo cell when at least `minCoverage` (½) of its area lies under the union of the three frames (exact for axis-aligned rectangles: inclusion–exclusion over the seven subsets); the bounding box of the logo cells exceeds the logo rectangle by at most one cell per side; `c = 0` gives no cells. Pure geometry in `core/brandLogo.ts`, unit-tested in Node.
- Draw order in the compositor: white → logo layer → grid lines → video in the open cells → overlays. The grid lines stay visible across the blocks (mock-up). The loop skips the layer while a mask is open and brings it back `reappearMs` (800) after the last open frame, so a short loss of the hands does not blink the logo; opening hides it at once. (The `inlay` variant of the plan, where only the open cells lose the logo, was not built: the user chose hide.)
- Style `wordmark`: navy blocks (`fill`) with the wordmarks on them: "Human" and "AI ETHIC CAMPAIGN" knocked out in white (`text`), "VERIFY:" in the brand green (`verify`). `wordmarkSvg()` derives the layer from the bundled SVG at runtime: every `<rect>` of a class with `fill:none` (the frames) is dropped so the frames merge into the blocks, `fill:#1f3566` → `text`, `fill:#99b43e` → `verify`, the live `<text>` with its `tspan`s becomes one flat `<text>` with `textLength="268"`, `lengthAdjust="spacingAndGlyphs"` and an inline `font-family` (Heavitas first, then 'Arial Black', 'Segoe UI Black', sans-serif), and the root gets `width`/`height` equal to the viewBox (Chrome's intrinsic size of an SVG without them is 300 × 150 scaled by the aspect, which shifts the source rectangle of `drawImage`). The wordmark is drawn at the fixed logo size (not quantized) and clipped to the logo cells, so at coarse grids the blocks cut its edges. The solid-blocks look (the plan's `silhouette`) remains as the fallback when the SVG does not load and as the `logoWordmark(0)` scenario for pixel tests.
- Not a camera pixel (I4): `mask/logoLayer.ts` renders one `OffscreenCanvas` per layout (blocks by `fillRect`, then the wordmark image clipped to the cells, one 9-argument `drawImage` from the `Image` of the SVG blob) and the compositor draws it with one 9-argument `drawImage`; `tools/check-invariants.mjs` lists the module with the same 9-argument rule (lint:boundaries keeps `mask/` away from the camera). Nothing leaves the browser (I9): the SVG is a `?raw` string in the bundle turned into a blob URL.
- Cost: cover closed → the logo is part of the static paint (the PERF-02 skip is unchanged: zero paints per second, measured +0 over 31 frames); cover open → nothing (hidden); a change of visibility or of the layer version (image decoded, wordmark scenario) is one repaint. The layer is rebuilt only when the layout object or the version changes; the geometry is cached per layout in a `WeakMap`.
- Settings: `ui.logo` (boolean, default `DEFAULTS.brand.logo.enabled` = true) in `uiState` (the tab's `wct.ui`, no epoch), switch "Logo trên màn che" (Logo on the cover) in the Interface section, mirrored into `LogoLayer.enabled`; `?logo=0|1` on the URL overrides the stored value for that load (`useUiState` overrides). Scenarios `logo(0|1)` (the switch) and `logoWordmark(0|1)`; `window.__wct.logo` and the `logo-stat` line of the debug drawer.

Steps:

0. Measurement: `tools/measure-logo.mjs` (raster export and mock-up) and the vector geometry read from the SVG; the raster of the first pass was replaced by the SVG as the bundled asset.
1. `core/brandLogo.ts` (pure): `LOGO_FRAMES_SVG`, `LOGO_STROKE`, `LOGO_CROP`, `LOGO_ASPECT`, `LOGO_FRAMES` (normalized), `logoRect(stage, board, placement)`, `coverage(frames, cell)`, `logoCells(rect, grid, minCoverage)`, `wordmarkSvg(source, colors)`; `DEFAULTS.brand.logo` (`enabled`, `widthRatio`, `maxHeightRatio`, `marginRatio`, `anchor`, `minCoverage`, `fill`, `text`, `verify`, `fonts`, `textLength`, `reappearMs`).
2. `mask/logoLayer.ts`: `LogoLayer` (SVG string in, `Image` from a blob URL, `ready`/`version`, `enabled`, `setWordmark`, `geometry(layout)` cached per layout, `draw(ctx, layout)` from a cached `OffscreenCanvas`).
3. Compositor: `LogoPainter` type, `paintBackground(ctx, layout, showLines, logo)` draws white → logo → lines, `RenderOptions.logo`.
4. Loop: `deps.logo()`, `lastOpenTs`, `logoVisible` (mask closed and `reappearMs` elapsed), `paintLogo`/`paintLogoVersion` in the static-paint key, `closeNow` resets, `LoopSnapshot.logoVisible`.
5. UI and debug: `uiState.logo`, `useUiState(defaults, overrides)`, the Interface switch, `?logo=`, strings in both dictionaries; `debug/logoProbe.ts` (`window.__wct.logo`, `describeLogo`), the `logo-stat` line, scenarios `logo` and `logoWordmark`; `StagePage` builds the layer from the `?raw` import and passes it to the loop, the first paint, the probe and the scenarios.
6. Style `wordmark`: `wordmarkSvg` (step 1) checked by the pixel cases of 7.33 and by eye at the three presets.
7. E2E helpers: `openApp` appends `logo=0` unless the query sets it and `APP_URL` accepts a query; `readLogo`; `logo.spec.ts` turns the logo on with `logo=1`. Tests of 7.33; documentation: README, 9.1, D-056, `tools/README.md`, `tools/test-report.mjs`.
8. Out of scope: a fade-in when the layer reappears and the same silhouette in the landing preview.

Done criteria: with the cover closed the logo blocks have the same px size in the three presets (e2e reads `__wct.logo.rect`), the cell bounding box is within one cell of the rectangle, a block cell is exactly the fill color with the wordmark off and the corners and mid-edges of the canvas stay white or grid gray; with the wordmark on the "VERIFY:" frame has green pixels and the "Human" frame white and navy pixels while nothing outside the rectangle has a logo color; while a region is open no logo pixel is drawn and `visible` is false; the logo returns `reappearMs` after the close and not before; the switch, `?logo=0` and the stored value behave as specified; every existing e2e passes unchanged; `paints` stays at zero with the cover closed and the logo still; no request leaves the origin.

Tests: section 7.33.

Implementation note (D-056, 2026-09-21): steps 0 to 7 done. Unit 307/307, e2e 78/78 (the four new cases measured: logo 288 × 109 px at all three presets on the headless stage, 25 / 97 / 445 cells, paints +0 over 31 static frames; 2,244 green px in the "VERIFY:" frame, 619 white and 4,506 navy px in the "Human" frame, 0 logo-colored px beside the rectangle; hidden 437 ms after the close and back at 913 ms against the 800 ms mark; gate audit 4 of 4 buffers clean). Open item: the campaign line is live text in Heavitas, which no target machine has, so it renders in 'Arial Black' squeezed to the original width; a re-export of the SVG with the text converted to outlines (Type → Create Outlines) restores the exact glyphs with no code change (`wordmarkSvg` leaves paths alone and only rewrites `<text>` elements).
...
### 7.32 Raised-finger participation tests (ROI-04)

| Test case | How it is checked | Type | Package |
|---|---|---|---|
| Fixtures | `tests/unit/fixtures/hands-pose.json` captured with `tools/probe-fingers.mjs --json` from the spike images (4 hands: two open hands, one pointing index, one thumbs-up), each hand labeled raised/folded per finger by eye; the webcam poses of the R-02 session are still to be added | fixture | ROI-04 |
| Pose classifier | Unit `fingerPose`: every fixture finger classified right on the first frame; the metrics lie in the measured ranges (with rounding tolerance); `fingerVote` band keeps the state both ways, thumb rule (abduction and palm polygon); debounce: two frames on the other side do not flip, the third does, a frame back resets the streak, `debounceFrames` 1 flips at once, a new track flips at once; 2D fallback without world landmarks gives the same result for the four fingers on the fixtures; missing landmarks or a zero-length palm keep the previous state or are raised on the first frame; `prev` is not mutated | unit | ROI-04 |
| Tracker state | Unit `handTracker` on the fixture detections: a new track is classified at once, a pose change needs three frames on the same id, `landmarksWorld` is copied; an uncertain frame (two same-label detections between two tracks) leaves the pose unchanged; a track dropped after 200 ms and re-created gets a new id and the new pose on its first frame | unit | ROI-04 |
| Fingertip reason | Unit `fingertips`: a folded tip has `reason: 'folded'` and is excluded; dominant reason order out-of-board > stale-point > folded > low-score; guidance names the number of folded fingers and, when all seen points are folded, asks to spread the fingers; `describeFingertips` counts `folded`; `raisedOnly` false reproduces ROI-03 | unit | ROI-04 |
| Store, config, i18n | Unit `store` (`raisedOnly` default true, a change → epoch++, the same value → no bump), `config` (threshold pairs low < high, `debounceFrames` × 50 ms ≤ point age), `i18n` (the new leaves exist in both dictionaries; the English folded sentence checked in `fingertips`) | unit | ROI-04 |
| Fake hands | Unit `fakeHands`: default hands carry 21 world landmarks and every finger raised; `raised` [4, 8] keeps tips 4 and 8 at (x, y ± spread / 2) and folds 12, 16, 20 into the palm polygon with ratio < 0.9 and angle < 120°, the index keeps ratio ≥ 1.05 and angle ≥ 135°, the thumb abduction ≥ 1; `raised` [] is a fist | unit | ROI-04 |
| Five-point hull | E2E `solver.spec`: fake hands with `raised` [4, 8, 12] left and [4, 8] right → a five-vertex `polygon` with 5 valid and 5 `folded` points, cell set equal to a recomputation in Node from the five raised tips; fold the right thumb → 4 vertices, fewer cells, same epoch; a fist on the right → `few-points` with the guidance "7 đầu ngón đang gập" and a white canvas; the `fingers-stat` line shows `trái 3/5 (2 folded) · phải 2/5 (3 folded)`; gate audit clean; record the hull and cell count | e2e | ROI-04 |
| Switch off | E2E `solver.spec` (same case): the `raisedOnly(0)` scenario unticks the "Chỉ ngón đang giơ" switch, epoch++, ten valid points and the cell set of all ten tips recomputed in Node with `raisedOnly: false`; ticking the switch again bumps the epoch and closes with `few-points`; every existing ROI-03 case passes unchanged with the default fake hands (all raised) | e2e | ROI-04 |
| Guidance and drawing | Unit `fingertips`: the folded count in the guidance in both languages, the all-folded sentence, `describeFingertips` with `folded`, out-of-board and stale-point before folded, `raisedOnly` false and a track without pose behave like ROI-03; the hollow marker of folded tips is checked by eye in the debug overlay (no pixel case) | unit | ROI-04 |
| Real hands (local) | E2E `hands.spec`: the ROI-03 case with `hands.jpg` still has 10 valid points (every finger raised); a new case draws `pointing_up.jpg` then `thumbs_up.jpg` at 1.5× in the synthetic scene with the point age at 1000 ms: one right hand with 21 world landmarks, pose raised [8] then [4], `few-points`, `phải 1/5 (4 folded)`, the guidance naming four folded fingertips; record the pose | local e2e | ROI-04 |
| Webcam calibration | R-02 session: open hand, fist, thumb folded over the palm, fingers toward the camera, back of the hand, one finger hidden by an object; record which rule decided each finger, adjust the thresholds in `DEFAULTS.hands.pose`, add the hands to the fixture; hold a 3 + 2 pose for 10 s without a flicker; note the results in D-055 | manual | ROI-04 |


### 7.33 Cover logo inlay tests (BRAND-01)

| Test case | How it is checked | Type | Package |
|---|---|---|---|
| SVG constants | Unit `brandLogo`: the three frames in code equal the `<rect>` attributes of the class with `fill:none` in the bundled SVG and `LOGO_STROKE` equals its `stroke-width`; both ink colors occur in the file; `LOGO_CROP` is (73.76, 90.91, 551.15 × 209.45), aspect 2.631; the normalized frames cover exactly [0, 1]² and sit top-right, middle-left, bottom-right | unit | BRAND-01 |
| Rectangle | Unit `brandLogo`: 30 % of the stage width, aspect, 2.5 % margin, top-left anchor → (32, 32, 384 × 146) on 1280 × 720, identical for the 32/64/128/256 16:9 grids; on a letterboxed 1000 × 600 stage the size and margin are equal at every preset and only the clamp to the board moves it; the five anchors; a board inset deeper than the margin clamps to the board; `maxHeightRatio` caps a wide logo; empty board or stage → null | unit | BRAND-01 |
| Coverage | Unit `brandLogo`: full cell 1, half cell 0.5, two frames on the same half still 0.5 (inclusion–exclusion), three frames tiling the cell 1, outside 0, empty cell 0 | unit | BRAND-01 |
| Cells | Unit `brandLogo`: about 97 cells at 64 cols (± 15 %), the 128 : 64 ratio between 3.3 and 4.7 and 64 : 32 between 3 and 5; the cell bounding box within one cell of the rectangle; no cell outside the board; a cell exactly half under the "Human" frame is in and 47.5 % is out; `c = 0`, an empty rectangle or a rectangle off the board → `EMPTY_CELLS` | unit | BRAND-01 |
| Wordmark SVG | Unit `brandLogo` (`wordmarkSvg` on the bundled file): the three frame rects are gone, the two green colon squares and every "Human" path remain, navy → `#ffffff`, green kept or replaced by the option, no `tspan`, one flat `<text>` "AI ETHIC CAMPAIGN" with the original class and transform, `textLength="268"`, `lengthAdjust`, inline Heavitas-first font stack; the root gains `width="700" height="400"`; the source string is unchanged | unit | BRAND-01 |
| Compositor order | Unit `compositor`: the logo painter is called once, right after the white fill and before the first grid line and the video `drawImage`; `logo: null` adds no call; with lines off and no mask the calls are exactly white then logo | unit | BRAND-01 |
| Config, uiState | Unit: `brand.logo` ranges (ratios in (0, 1], anchor in its set, `minCoverage` in (0, 1], three `#rrggbb` colors, Heavitas-first font stack, `textLength` > 0, `reappearMs` ≥ 0); `readUiState` with `logo` (default, boolean kept, garbage → default, old records without the key) | unit | BRAND-01 |
| Fixed size | E2E `logo.spec` (`?logo=1`, cover closed, scenario `logoWordmark(0)`): at the three presets `__wct.logo.rect` has the same width and height and moves less than one coarse cell, the cell bounding box is within one cell of it, the rectangle is 30 % of the stage width at the 2.5 % margin clamped to the board; the cell containing (12 %, 57 %) of the rectangle is exactly `#1f3566`; the cell beside the bounding box and the nine samples of `expectCanvasWhite` are white or grid gray; cell counts recorded; `paints` +0 over 20 loop frames | e2e | BRAND-01 |
| Wordmark | E2E `logo.spec` (`logoWordmark(1)`): inside the "VERIFY:" frame more than 50 green px, inside the "Human" frame more than 50 white and 200 navy px and no green, a 40 px strip right of the rectangle has no logo color; no request leaves the origin (blob URLs are in-page); pixel counts recorded | e2e | BRAND-01 |
| Hidden while open | E2E `logo.spec`: a mouse window far from the logo → `visible` false and the block cell white, `logo-stat` says "ẩn (vùng mở)"; `coverAll` → the block cell is still white while the close is younger than the mark and the logo is back at ≥ 750 ms with the cell navy again; gate audit clean over the open phase; the two times recorded | e2e | BRAND-01 |
| Switch and URL | E2E `logo.spec`: the "Logo trên màn che" switch off → `enabled` false, `logoVisible` false, the block cell white, `logo-stat` "logo: tắt", `wct.ui.logo` false; a reload in the same tab without the parameter keeps it off; `?logo=1` turns it on and `?logo=0` off (canvas white); the English label "Logo on the cover" with `?lang=en` | e2e | BRAND-01 |
| Existing suite | Every existing e2e passes unchanged: `openApp` adds `logo=0` and `APP_URL` accepts the query; the `synthetic.spec` scenario list gains `logo` and `logoWordmark`; the landing-page start flows (logo on, top-left) still pass `expectCanvasWhite` because its nine samples never touch the corner logo | e2e | BRAND-01 |
| Raster cross-check | `tools/measure-logo.mjs <export.png> [mockup.png]` on a raster export reproduces the frame strokes, the two ink colors and the wordmark bounds of the BRAND-01 table; on the mock-up the cell pitch and the block bounding box | tool | BRAND-01 |
| By eye | Webcam session: the wordmark readable at the three presets at 1280 px and on the kiosk display; block edges on the cell lines; no blink when the hands come and go; the campaign line after the outlined re-export | manual | BRAND-01 |
...
export const DEFAULTS = {
  ...
  brand: { logo: { enabled: true, widthRatio: 0.3, maxHeightRatio: 0.6, marginRatio: 0.025, anchor: 'top-left', minCoverage: 0.5,
                  fill: '#1f3566', text: '#ffffff', verify: '#99b43e', textLength: 268, reappearMs: 800 } },  // BRAND-01 (D-056)
} as const;
```
