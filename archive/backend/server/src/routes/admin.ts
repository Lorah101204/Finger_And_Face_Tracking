// ADM-01: đăng nhập admin, bảng phiên, chi tiết sự kiện, thống kê, xuất CSV.
...
export function registerAdminRoutes(app: FastifyInstance, ctx: AppContext): void {
  ...
  function adminFromRequest(req: FastifyRequest) {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (!ok || !user) return reply.code(401).send({ error: 'bad_credentials' })
  ...
  app.post('/api/admin/logout', async (req, reply) => {
    ...
    if (raw) {
      const unsigned = req.unsignCookie(raw)
      if (unsigned.valid && unsigned.value) repo.deleteAdminSession(hashToken(unsigned.value))
    }
