// API-00, LOG-01: phiên khách ẩn danh, đồng ý, nhận sự kiện metadata (bất biến I9, I10).
...
function toPublic(s: SessionRow) {
  return {
    sessionId: s.id,
...
export function registerSessionRoutes(app: FastifyInstance, ctx: AppContext): void {
  ...
  function setSidCookie(reply: FastifyReply, id: string) {
  ...
  function currentSession(req: FastifyRequest): SessionRow | null {
    ...
    if (Date.now() - Date.parse(s.last_seen_at) > ttlMs) return null
    return s
  }
  ...
  app.post('/api/session/start', async (req, reply) => {
    ...
    return toPublic(repo.getSession(s.id) as SessionRow)
  })

  app.get('/api/session/me', async (req, reply) => {
    const s = currentSession(req)
    if (!s) return reply.code(401).send({ error: 'no_session' })
    return toPublic(s)
  })
